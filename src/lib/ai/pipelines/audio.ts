...
export async function runAudioStep(
  ...
  if (step === "audio_director") {
    ...
    const segmentTranscripts = buildSegmentTranscriptMap(audioPlan, audioDirection);
    ...
    return {
      ...
      metadataPatch: mergeMetadata(metadata, {
        ...
        audioDirection,
        checkpoint: { audioDirection, segmentTranscripts },
        audio: {
  ...
  if (step === "audio_tts") {
    ...
    const existingForSegment = segmentAssets.find(
    ...
    if (existingForSegment) {
      ...
      return {
        ...
        metadataPatch: mergeMetadata(metadata, {
          ...
          checkpoint: { segmentAssets: dedupeSegmentAssets(segmentAssets) },
        }),
      };
    }

    let segmentTranscript = transcriptForSegment(
      audioDirection,
      segment,
    ...
    segmentTranscript = guardSegmentTranscript({
      segmentTranscript,
      segmentId: segment.segmentId,
      segmentIndex,
      totalSegments: segments.length,
      fullTranscript: audioDirection.transcript,
      creationId: creation.id,
    });
  ...
  if (step === "audio_compose") {
    ...
    const segmentAssets = dedupeSegmentAssets(checkpoint.segmentAssets ?? []);

    if (!audioPlan || !audioDirection) {
      throw new Error("Missing checkpoints for audio compose");
    }

    const existingManifestAsset = await findCreationAssetByRole(
      creation.contentKey,
      creation.id,
      "audio_manifest",
    );
    ...
    if (existingManifestAsset) {
      const manifestBuffer = await downloadAssetBuffer(
        existingManifestAsset.storage_path,
      );
      const manifest = audioManifestSchema.parse(
        JSON.parse(manifestBuffer.toString("utf-8")),
      );
      const measuredMinutes =
        manifest.actualDurationMinutes ?? audioPlan.targetDurationMinutes;
      ...
      return {
        ...
        metadataPatch: mergeMetadata(metadata, {
          ...
          display: {
            excerpt,
            ...(manifest.visualTheme
              ? { visualTheme: manifest.visualTheme }
              : {}),
          },
          audio: {
            format: audioPlan.format,
    ...
    if (wavBuffers.length > 0) {
      ...
      if (shouldSkipMasterUpload(masterWav.byteLength)) {
        console.warn(
          "[audio-compose] master WAV exceeds upload limit, using segment playback",
          {
            creationId: creation.id,
            byteLength: masterWav.byteLength,
            segmentCount: segmentAssets.length,
          },
        );
      }

      let masterMp3: Buffer | undefined;
      try {
        masterMp3 = encodeWavBufferToMp3(masterWav, creation.id);
      } catch (error) {
        console.warn("[audio-compose] MP3 encode failed, falling back to WAV master", {
          creationId: creation.id,
          error: error instanceof Error ? error.message : String(error),
        });
      }

      if (masterMp3 && !shouldSkipMasterUpload(masterMp3.byteLength)) {
        ...
        !shouldSkipMasterUpload(masterWav.byteLength) &&
        !masterMp3
      ) {
